...
[Русский](recognition-lab.md) | **English**
...
Implemented: paired-data validation, nineteen comparison profiles, offline and
real-time replay, metrics, local audio conversion, verified-pair export and
pause-aware segmentation with timestamp-owned overlap.

Not yet completed: a broader held-out real-audio benchmark, automated live-device
latency tests, model fine-tuning and deployment of a trained model.
...
- `pause_preview_off`: disable the fast provisional result.
- `pause_initial_6`: increase the initial forced flush to 6 seconds.
- `pause_initial_8`: increase the initial forced flush to 8 seconds.
