...
$url = "http://127.0.0.1:$Port/speaker.html"
$healthUrl = "http://127.0.0.1:$Port/health"

function Test-SpeakerServer {
    param([string]$Uri)
...
if (-not (Test-SpeakerServer -Uri $healthUrl)) {
    ...
    for ($i = 0; $i -lt 40; $i++) {
        if (Test-SpeakerServer -Uri $healthUrl) {
            break
        }
...
if (-not (Test-SpeakerServer -Uri $healthUrl)) {
    throw "Speaker Text server did not start on $url"
}
